...
\begin{tcbdfn}
Let $(A, \delta, \varepsilon)$, $(A', \delta', \varepsilon')$ be $\Bbbk$-coalgebras. A \emph{$\Bbbk$-coalgebra homomorphism} $\varphi \colon A \to A'$ is a $\Bbbk$-linear map that preserves the comultiplication and counit structures.
Concretely, the following diagrams are required to commute.
\[
\begin{tikzcd}
    A \tensor A \arrow[r, "\varphi \tensor \varphi"] & A' \tensor A' \\
    A \arrow[r, "\varphi"] \arrow[u, "\delta", swap] & A' \arrow[u, "\delta'", swap]
\end{tikzcd}
\qquad
\begin{tikzcd}
    A \arrow[rr, "\varphi"] \arrow[dr, "\varepsilon", swap] & & A' \arrow[dl, "\varepsilon'"]\\
    & \Bbbk &
\end{tikzcd}
\]
\end{tcbdfn}
...
\begin{tcbdfn}[Frobenius algebras]
    ...
    \begin{enumerate}[(i)]
        \item A linear form $\varepsilon \colon A \to \Bbbk$ whose kernel contains no nontrivial (left) ideals. [See definition \ref{dfn:frobenius-algebra-a}]
        \item A non-degenerate associative pairing $\beta \colon A \tensor A \to \Bbbk$ (and a copairing $\gamma \colon \Bbbk\to A \tensor A$). [See definition \ref{dfn:frobenius-algebra-b}]
        \item A coalgebra structure $(A, \delta, \varepsilon)$ satisfying the Frobenius relation. [See definition \ref{dfn:frobenius-algebra-c}]
    \end{enumerate}
\end{tcbdfn}
...
The aim of this section is to define a graphical language that will help us see more clearly the equivalence between the various definitions of Frobenius algebras. We hence begin by drawing the maps defining a $\Bbbk$-algebra as \emph{wires}.
...
\end{tikzpicture}
\]
Through this section, we'll adopt these drawings as a formal notations corresponding to $\Bbbk$-linear maps $A^m \to A^n$. The tensor product of two maps will correspond to placing the diagrams \emph{in parallel} (one above the other), while the composition will be represented by connecting them \emph{in series} (by simply connecting the wires).
