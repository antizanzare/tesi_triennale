...
Furthermore, we can check how this operation defines a monoidal structure $(\Delta, +, [0])$.\todo{check??} We hence investigate generators and relations starting from some known \todo{add homology reference} properties of $\Delta$\footnote{Alternatively we could completely define $\Delta$ graphically, without reliying on homology. Such approach can be found in [Kock]}. 
...
These axioms carry a striking resemblance to the ones we drew above, when describing the internal monoid. We can hence state that $([1], \mu^{(0)}, \mu^{(2)})$ is a monoid in the monoidal category $(\Delta, +, [0])$. Since this monoid completely generates the whole category we have the following lemma.
\begin{tcblemma}
    $(\Delta, +, [0])$ is the free monoidal category on a single monoid, namely $([1], \mu^{(0)}, \mu^{(2)})$.
\end{tcblemma}
...
\begin{proof}
    \todo[inline]{riscrivere tutto in modo più chiaro}
    Given a monoidal functor $F \colon \Delta \to \cat{C}$, we can just evaluate it on the monoid $[1]$. Hence by $\ref{lem:monoid_image}$ we obtain an internal monoid $(F([1]), F(\mu^{(0)}), F(\mu^{(2)}))$ in $\cat{C}$. We now take a monoidal natural transformation $u \colon F \Rightarrow G$ and consider its component $u_{[1]} \colon F([1]) \to G([1])$, which by monoidality of $u$ determines all the other components (we'll have $u_{[2]}= u_{[1]} \sq u_{[1]}$, and so on). Now, since $u$ is also a natural transformation we have the compatibility of $u_{[1]}$ with all the maps in $\Delta$, all generated by $\mu^{(0)}$ and $\mu^{(2)}$. This implies that $u_{[1]}$ defines a morphism of internal monoids in $\cat{C}$.

    Conversely, given a monoid $(M, \eta, \mu)$ in $\cat{C}$, we define a monoidal functor $\Delta \to \cat{C}$ as follows
    \begin{align*}
        \Delta & \to \cat{C} \\
        ([0] \to [1]) & \mapsto (\eta \colon I \to M) \\
        ([2] \to [1]) & \mapsto (\mu \colon M \sq M \to M)
    \end{align*} 
\end{proof}
...
\begin{tcbdfn}[Internal Frobenius object]
Let $(\cat{C}, \sq, I)$ be a monoidal category.
\end{tcbdfn}
