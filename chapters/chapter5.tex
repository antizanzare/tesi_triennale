...
\begin{tcbex}
An internal monoid in $(\mathbf{Set}, \disj, \varnothing)$ is a monoid. An internal comonoid in  $(\mathbf{Set}, \disj, \varnothing)$ is a comonoid.
...
\begin{tikzpicture}[x=0.6cm, y=0.3cm, baseline]
    ...
    \draw (2,-3)--(3,-3)--(4,0);
    \draw (2,-5)--(4,-5);
    \draw[rounded corners=0.3cm] (4,2) -- (5,2) -- (5,1);
...
\text{etc}
\]
\ref{lem:monoidal-falg} \todo{da finire}
\end{proof}
...
\begin{tcblemma}\label{lem:monoid_image}
    The image of a monoid through a monoidal functor is a monoid 
    \todo[inline]{scrivi bene lo statement}
\end{tcblemma}
...
Furthermore, we can check how this operation defines a monoidal structure $(\Delta, +, [0])$.\todo{check??} We hence investigate generators and relations starting from some known \todo{add homology reference} properties of $\Delta$
\todo{sistemare la footnote}
\footnote{Alternatively we could completely define $\Delta$ graphically, without reliying on "homology". Such approach can be found in [Kock]}. 
...
The monoidal category $(\Delta, +, [0])$ is completely generated by the morphisms 
    % \[ 
    % \mu^{(0)} \colon [0] \to [1] 
    % \quad \text{and} \quad 
    % \mu^{(2)} \colon [2] \to [1] 
...
\end{tcblemma}
\todo[inline]{a free?? unique up to ???} 
...
Let $(\cat{C}, \sq, I)$ be a monoidal category. An \emph{internal Frobenius object} in $\cat{C}$ is an object $M$ equipped with four morphisms
    \[
    \mu \colon M \sq M \to M \quad\text{(multiplication)} 
    \hspace{3em}
    \eta \colon I \to M \quad\text{(unit)}
...
\todo[inline]{graphical representation}

\begin{tcblemma}
    An internal Frobenius object $M$ is both an internal monoid and an internal comonoid. 
\end{tcblemma}
\begin{proof}
    Let $(M, \mu, \eta, \delta, \varepsilon)$ be a Frobenius object. By proving $\mu$ and $\delta$ are associatives, we obtain an internal monoid $(M, \mu, \eta)$ and an internal comonoid $(M, \delta, \varepsilon)$. \todo{prove}
\end{proof}

Following this lemma, we define \emph{homomorphisms of internal Frobenius objects} to be morphisms $\varphi \colon (M, \mu, \eta, \delta, \varepsilon) \to (M', \mu', \eta', \delta', \varepsilon')$ that are homomorphisms of both the underlying monoid structure and the comonoid one. In this way, given any monoidal category $(\cat{C}, \sq, I)$, we obtain a category $\mathbf{Frob}(\cat{C})$ whose objects are internal Frobenius objects in $\cat{C}$ and arrows are the homomorphisms between them.
...
\begin{tcbdfn}[A free monoidal category on an internal Frobenius object]
    Let $(\cat{F}, \sq, 0)$ be the monoidal category whose objects are the monoidal powers of a given object $F$ and whose arrows are generated by 
    \[
    \begin{tikzpicture}[x=0.8cm, y=0.4cm, baseline]
    \draw[fill=black] (0,.5) circle (2pt);
    \draw[fill=black] (0,-.5) circle (2pt);
...
\begin{tcbdfn}[Commutative Frobenius objects]
    An internal Frobenius object $(M, \mu, \eta, \delta, \varepsilon)$ in a symmetric monoidal category $(\cat{C}, \sq, I, \beta)$ is \emph{commutative} if the twist commutes with the multiplication $\mu$.
\end{tcbdfn}
\begin{tcbdfn}[Cocommutative Frobenius objects]
    An internal Frobenius object $(M, \mu, \eta, \delta, \varepsilon)$ in a symmetric monoidal category $(\cat{C}, \sq, I, \beta)$ is \emph{cocommutative} if the twist cocommutes with the comultiplication $\delta$.
\end{tcbdfn}
