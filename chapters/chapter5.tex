...
Thanks to our journey through the previous chapters the proof becomes straightforward. We can now explicitly build the correspondence, for both objects and arrows.
...
Since every arrow in $\bord{2}$ is built by tensoring and gluing the generators, the compatibility of $u$ with the arrows in $\bord{2}$ is entirely determined by the corresponding diagrams.
...
Let us put everything in the right context by giving some more definitions from category theory.

\begin{tcbdfn}[Internal monoids]
 \label{dfn:internal-monoid}
 Let $(\cat{C}, \sq, I)$ be a strict monoidal category. An \emph{internal monoid}\index{internal monoid} in $\cat{C}$ is an object $M$ equipped with two morphisms
 \[
...
By reversing the arrows, we also give the definition of \emph{internal comonoid}. The definition extends analogously to the case of weak monoidal categories.

\begin{tcbdfn}[Internal comonoids]
 \label{dfn:internal-comonoid}
 Let $(\cat{C}, \sq, I)$ be a strict monoidal category. An \emph{internal comonoid}\index{internal comonoid} in $\cat{C}$ is an object $M$ equipped with two morphisms
 \[
...
Both these structures can be graphically represented, similarly to what we did in the case of $\Bbbk$-Frobenius algebras.
...
\begin{tcbex} \par
 ...
 \par
 An  internal monoid in $(\mathbf{Cat}, \times, \mathbf{1})$, where $\mathbf{1}$ is a category with only one object, is a monoidal category. Although we have not done so, we can also define a comonoidal category, which will exactly be the comonoid in $(\mathbf{Cat}, \times, \mathbf{1})$
\end{tcbex}
...
\begin{tcbdfn}[Homomorphisms of internal monoids]
 Let $(\cat{C}, \sq, I)$ be a strict monoidal category. An \emph{homomorphisms of internal monoids} $\varphi \colon (M, \mu, \eta) \to (M', \mu', \eta')$ is a morphism from $M$ to $M'$ such that the following diagrams commute.
 \[
  \begin{tikzcd}
...
\begin{tcbdfn}[Homomorphisms of internal comonoids]
 Let $(\cat{C}, \sq, I)$ be a strict monoidal category. An \emph{homomorphisms of internal comonoids} $\phi \colon (M, \delta, \varepsilon) \to (M', \delta', \varepsilon')$ is a morphism from $M$ to $M'$ such that the following diagrams commute.
 \[
  \begin{tikzcd}
...
\begin{tcbdfn}[Commutative internal monoids]
 Let $(\cat{C}, \sq, I, \beta)$ be a symmetric monoidal category. An internal monoid $(M, \mu, \eta)$ in $\cat{C}$ is \emph{commutative}\index{internal monoids!commutative internal monoids} if its multiplication is compatible with the twist map. Equivalently we are asking for the following diagram to commute.
 \[
  \begin{tikzcd}
...
Similarly to the sum we are accustomed to, the ordinal sum we just defined has $[0] = \varnothing$ as the neutral element.

Furthermore, this operation defines a monoidal structure $(\Delta, +, [0])$. We hence investigate generators and relations starting from some known properties of $\Delta$
\footnote{Alternatively we could completely define $\Delta$ graphically, without reliying on simplicial relations. Such approach can be found in \cite{kock2003frobenius}}. We again refer the reader to \cite{maclane}.
...
\begin{proof}
 ...
 Conversely, given a monoid $(M, \mu, \eta)$ in $\cat{C}$, we define a monoidal functor $F_M \colon \Delta \to \cat{C}$ as follows
 \begin{align*}
...
\begin{remark}
 ...
 \[
  ...
  \end{tikzpicture}
 \]
 satisfying the relations specular to the ones in Lemma~\ref{lem:graphical-delta}. We then have the canonical equivalence of categories
 \[
  \mathbf{MonCat}(\Delta^{op}, \cat{C}) \iso \mathbf{Comon}(\cat{C})
 \]
\end{remark}

We still have not highlighted the connection between these general results and the equivalence we stated in Theorem~\ref{thm:equiv}. To do so we just need to choose the right monoidal category. Indeed by taking $(\cat{C}, \sq, I)$ equal to $(\mathbf{Vect}_\Bbbk, \tensor, \Bbbk)$ we obtain
\[
...
\end{tcblemma}
\begin{proof}
 Let $(M, \mu, \eta, \delta, \varepsilon)$ be a Frobenius object. By proving $\mu$ and $\delta$ are associative, we obtain an internal monoid $(M, \mu, \eta)$ and an internal comonoid $(M, \delta, \varepsilon)$. The proof of this fact is equivalent to the graphical proof of the equivalence of Frobenius algebras we gave in the previous chapter (see Proposition~\ref{prp:Frobenius-equivalence}).
\end{proof}
...
\begin{proof}
 Let us take two internal Frobenius object $(M, \mu, \eta, \delta, \varepsilon)$, $(M', \mu', \eta', \delta', \varepsilon')$ and prove the multiplication defined in Lemma~\ref{lem:product-of-monoids} together with the analogous comultiplication define an internal Frobenius object $M \sq M'$. What we need to do is proving the Frobenius relation holds.
 \[
  \begin{tikzpicture}[x=0.6cm, y=0.3cm, baseline]
   \draw (0,-5) -- (4,-5);
   \draw (0,-3) -- (3,-3);
