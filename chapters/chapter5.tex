...
\begin{tcbdfn}[Internal monoids]
    ...
    Let $(\cat{C}, \sq, I)$ be a monoidal category. An \emph{internal comonoid} in $\cat{C}$ is an object $M$ equipped with two morphisms
    \[
...
\begin{dfnx*}[Internal monoids]
Let $(\cat{C}, \sq, I)$ be a monoidal category. An \emph{internal monoid} in $\cat{C}$ is an object $M$ equipped with two morphisms
    \[ 
...
Let $(\cat{C}, \sq, I, \beta)$ be a symmetric monoidal category. An internal monoid $(M, \mu, \eta)$ in $\cat{C}$ is \emph{commutative} if its multiplication is compatible with the twist map. Equivalentely we are asking for the following diagram to commute.
\[
\begin{tikzcd}
...
\todo[inline]{cocommutative internal comonoid}
...
\footnote{Alternatively we could completely define $\Delta$ graphically, without reliying on [ref]. Such approach can be found in [Kock]}. 
...
The monoidal category $(\Delta, +, [0])$ is completely generated in the sense of Definition \ref{dfn:generators} by the morphisms 
    % \[ 
    % \mu^{(0)} \colon [0] \to [1] 
    % \quad \text{and} \quad 
    % \mu^{(2)} \colon [2] \to [1] 
