...
Furthermore, we can check how this operation defines a monoidal structure $(\Delta, +, [0])$.\todo{check??} We hence investigate generators and relations starting from some known properties of $\Delta$. 

Being $[1]$ a terminal object we have a unique arrow
\[ \mu^{(n)} \colon [n] \to [1] \hspace{2em} \text{for every $[n] \in \obj{\Delta}$}\]

Face degeneracy generators and relations.
