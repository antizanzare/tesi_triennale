...
\begin{tcbprp}\label{prp:cocartesian-monoidal}
    Let $\cat{C}$ be a category that admits coproducts. Then $(\cat{C}, +, 0)$, where $+$ denotes the coproduct and $0$ the initial object, is a monoidal category.
\end{tcbprp}
We call this structure a \emph{cocartesian} monoidal category.
% \fouche{Una importante differenza con le cat monoidali generiche però è che `Being cartesian or cocartesian monoidal' è una proprietà, non una struttura: c'è un solo modo di avere i prodotti (soddisfare la prop univ), ma non c'è un solo modo di definire $\sq$ e $I$ (del resto, $I$ è unico up to iso quando la struttura monoidale è specificata, analogo a `l'unità di un monoide è unica'). E' uno dei primi teoremini in EGNO, la cui dimostrazione è un parto.}
% \fouche{Capisci, questo è il genere di dettagli che ti avrei fatto studiare: sono al tuo livello e sono educativi.}
\begin{tcbex}
    ...
    \begin{itemize}
        ...
        \item $(\Delta, {+}, [0])$, the strict monoidal simplicial category
    ...
    \end{itemize}
\end{tcbex}
%\fouche{$(\Delta, {+}, [0])$ non è cocartesiana; la somma ordinale non è il coprodotto, sebbene agisca come l'unione disgiunta sugli oggetti... Also: ciascuna di queste categorie monoidali ha una proprietà universale, forse sarebbe meglio menzionarla e inserire anche un esempio di $\sq$ non universale?}
...
\begin{tcbdfn}[Monoidal functors]\label{dfn:monoidal-functor}
    Let $(\cat{C}, \sq, I, \alpha^\cat{C}, l^\cat{C}, r^\cat{C})$ and $(\cat{D}, \varsq , J, \alpha^\cat{D}, l^\cat{D}, r^\cat{D})$ be two (weak) monoidal categories. A monoidal functor is a triple $(F, \varphi, \varepsilon)$ consisting of
    \begin{itemize}
...
\begin{remark}
    ...
    % \fouche{no, questa non è la definizione di oplax monoidal functor: un conto è rovesciare l'ordine del prodotto $\sq$ definendo $\sq^* : \cat{C}\times\cat{C} \to \cat{C}$ come $(A,B)\mapsto B\sq A$ invece che $A\sq B$ (per esempio questi due sono diversi quando...), e un altro conto del tutto diverso è considerare la categoria $\cat{C}^{op}$ col prodotto monoidale $\sq^{op} : (\cat{C}\times\cat{C})^{op} = \cat{C}^{op} \times \cat{C}^{op} \to \cat{C}^{op}$; purtroppo c'è un clash di notazione in questo contesto, perché $(\cat{C}, \sq^*)$ consiste della bicategoria con un solo oggetto $(\Sigma\cat{C})^{1op}$ dove è stata rovesciata la direzione di composizione delle 1-celle (cioè degli oggetti di $\cat{C}$, che si compongono via $\sq$), mentre $(\cat{C}^{op}, \sq^{op})$ consiste della bicategoria con un solo oggetto $(\Sigma\cat{C})^{2op}=\Sigma(\cat{C}^{op})$ ---molto più spesso chiamata $(\Sigma\cat{C})^{co}$--- dove è stata rovesciata la direzione di composizione delle 2-celle (cioè delle frecce di $\cat{C}$). Le due non sono la stessa cosa.}
    In this case we will have natural transformations $\varphi_{A, B} \colon F(A \sq B) \to F(A) \varsq F(B)$ and $\varepsilon \colon F(I) \to J$. The idea behind these two notions is that a \emph{lax} monoidal functor is the one preserving the structure of internal monoid, while an \emph{oplax} monoidal functor preserves the internal comonoid structure. The definition of an internal monoid (resp. comonoid) will be properly given in \ref{dfn:internal-monoid} (resp. \ref{dfn:internal-comonoid}).
\end{remark}

\begin{tcbdfn}[Strong monoidal functors]
    Let $(\cat{C}, \sq, I)$ and $(\cat{D}, \varsq , J)$ be two (weak) monoidal categories. A monoidal functor $(F, \varphi, \varepsilon)$ from $\cat{C}$ to $\cat{D}$ is \emph{strong monoidal}\index{monoidal category!strong monoidal functor} if $\varphi$ and $\varepsilon$ are isomorphism.
\end{tcbdfn}
...
As the reader can imagine, working with weak monoidal categories can become a really tedious process. The following result mitigates this difficulty, justifying working with monoidal categories \emph{as if they were strict}, forgetting about associators and unitors without losing generalities. The proof and a more in depth explanation can be found in \cite{maclane} and \cite{coherence}. 
\end{remark}
% \begin{remark}\label{rmk:strictification}
...
\end{tcbthm}
Essentially, what follows is that from now on we can assume, without loss of generalities, that monoidal categories \emph{are} strict. This is possible because the equivalence provided by the above theorem ensures that all coherence diagram commute up to unique canonical isomorphism, as long as we don't require functors between such categories to be strict. To do so, from now on, when considering the functor category $\mathbf{MonCat}$ we will referring to $\mathbf{MonCat}_{\textup{strong}}$.
...
\begin{remark}
    ...
    \end{tikzcd}
    \]
    By naturality of $u$, one checks the two diagrams commute without asking further conditions to hold. We hence have that every natural transformation between two functors defined on cartesian monoidal categories are always monoidal. The same holds when the two categories are cocartesian monoidal.
\end{remark}
...
% Given two monoidal categories $(\cat{C}, \sq, I)$, $(\cat{D}, \varsq, J)$ we can then consider the category $\moncat(\cat{C}, \cat{D})$\index{monoidal category!monoidal functor category} where objects are strong \todo{!!!} monoidal functors from $\cat{C}$ to $\cat{D}$ and arrows are monoidal natural transformations between such functors.
...
Let us show some example of symmetric monoidal categories.
...
\begin{tcbex}[Free symmetric monoidal category on a single object]
    ...
    \end{itemize}
    The monoidal operation $+$ is defined by juxtapposition\footnotemark, with the empty set $\varnothing = [0]$ as the unit object. The symmetric structure is defined by interchanging factors (we define the \emph{twist} from $[m]+[n]$ to $[n]+[m]$ by the association $\{0_m, \dots, (m-1)_m, 0_n, \dots,  (n-1)_n\} \mapsto \{0_n, \dots, (n-1)_n, 0_m, \dots, (m-1)_m\}$).
\end{tcbex}
...
\begin{tcbex}[Free braided monoidal category on a single object]
    Similarly to the previous case, we define the \emph{braided monoidal category} $(\mathbf{Braid}, +, [0])$ by taking the braid groups $\text{Braid}(n)$ as morphisms.
\end{tcbex}

It is important to understand that being a symmetric monoidal category involves defining a strucutre, not merely posssessing a property. This distinction is clearly shown by the following example.
