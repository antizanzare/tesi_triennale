...
\item \fouche{Perché a rombo, perché non semplicemente un quadrato?}
\[
\begin{tikzcd}
...
\end{figure}
\fouche{Anche piu piccolo sto pentagono ziobon}
\begin{figure}
\centering
\begin{tikzcd}
...
\begin{tcbex}
    ...
    \begin{itemize}
        ...
        \item $(\Delta, {+}, [0])$, the strict cocartesian monoidal simplicial category
...
\begin{remark}
    ...
    In this case we will have natural transformations $\varphi_{A, B} \colon F(A \sq B) \to F(A) \varsq F(B)$ and $\varepsilon \colon F(I) \to J$. The idea behind these two notions is that a \emph{lax} monoidal functor is the one preserving the structure of internal monoid, while an \emph{oplax} monoidal functor preserves the internal comonoid structre. The definition of an internal monoid (resp. comonoid) will be properly given in \ref{dfn:internal-monoid} (resp. \ref{dfn:internal-comonoid}).
\end{remark}
