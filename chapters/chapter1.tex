...
\begin{tcbdfn}[Monoidal categories] \label{dfn:monoidal_category}
 ...
 \end{itemize}
 These maps must satisfy the so-called coherence requirements, depicted in Figure~\ref{fig:coherence_pentagon} and Figure~\ref{fig:coherence_unit}, for every $A,B,C,D \in \obj{\cat{C}}$.
\end{tcbdfn}
...
\begin{tcbex}
 ...
 \begin{itemize}
  ...
  \item $(\Delta, {+}, [0])$, the strict monoidal simplicial category. A more in-depth description of it can be found in Definition~\ref{dfn:delta-cat}.
...
Again, by Theorem~\ref{thm:strictification}, when considering braided monoidal categories, we can restrict to the following definition.
...
\end{tcbex}
\footnotetext{We can see an explicit definition of such operation in Definition~\ref{dfn:ordinal-sum}, for the case of $\Delta$}
