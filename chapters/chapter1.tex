...
\begin{remark}
 The strictification of the monoidal category of finite dimensional vectors spaces over a field $k$ is the category (cf. \cite{}) $\mathbb{N}[k]$ whose objects are iterated powers $k^n$ of $k$ with itself; the monoidal structure is given by product of natural numbers, i.e. as the strict equality 
 \[k^n\oplus k^m = k^{nm}\]
 Note that $\mathbb{N}[k]$ is a PROP in the sense of \cite{}; the sum of natural numbers gives $\mathbb{N}[k]$ another monoidal structure $\oplus$, in that 
 \[k^n \oplus k^m = k^{n+m}\]
