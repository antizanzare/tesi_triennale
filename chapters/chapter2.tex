...
Let $V$ be a finite dimensional real vector space. We say that two ordered basis $\mathcal{B}_1$, $\mathcal{B}_2$ have the \emph{same orientation} (resp. \emph{opposite orientation}) if the linear transformation carrying one into the other has positive (resp. negative) determinant. An \emph{orientation} on $V$ is given by associating a sign (either $+$ or $-$) to each ordered basis, following the rule just stated.  
\end{tcbdfn}

\begin{tcbdfn}[Linear maps and orientation]
Let $V$, $W$ be two oriented, finite dimensional real vector spaces. A linear map $f \colon V \to W$ is \emph{orientation preserving} if it sends positive basis into positive basis and \emph{orientation reversing} if positive bases are sent into negative ones.
\end{tcbdfn}

% \begin{tcbdfn}[Oriented manifolds]
% An \emph{orientation} of a (smooth) manifold $M$ is a choice of orientation of its tangent bundle $TM$.  In making such choice we require that the differentials of the transition functions preserve orientations.  We say a smooth manifold $M$ is \emph{orientable} when it admits an orientation.
% \end{tcbdfn}
\begin{tcbdfn}[Oriented manifolds]
...
\begin{example}
  Take a circle $\mathbb{S}^1$ with the usual counterclockwise orientation and the interval $I \defeq [0,1]$ with its standard orientation. The products $\mathbb{S} \times I$ and $I \times \mathbb{S}$ then have opposite orientations. We hence need to be careful when choosing the order of such factors.
  \[
...
\begin{tcbdfn}[The induced orientation on a boundary]
  Let $M$ be an oriented manifold with boundary. Its boundary $\partial M$ inherits a canonical orientation, defined as follows. Consider an outward pointing vector $n \in T_x M \setminus T_x (\partial M)$; for any basis $\{ t_1, \dots, t_n\}$ of $T_x (\partial M)$ we say it to be a $\emph{positive}$ (resp. \emph{negative}) if $\{n, t_1, \dots, t_n\}$ is a positive (resp. negative) basis for $T_x M$.
\end{tcbdfn}
...
\todo[inline]{importante! questo è scritto male/falso. i bordi hanno la loro orientazione. Devo poter definire il "duale"}
\begin{example}
Let us draw an example of a 2-dimensional bordism. We consider a 2-dimensional oriented manifold $M$ homeomorphic to a spere with three holes (also called a \emph{pair of pants}) and the corresponding maps $i_0$, $i_1$, defining its orientation as a bordism.
\begin{figure}[ht]
  \centering
   \begin{tikzpicture}[every tqft/.style={transform shape}]
    \pic[name=a, tqft, offset=-0.7, incoming boundary components=1,
    outgoing boundary components=2, anchor=between outgoing 1 and 2, at={(0,-1.5)}, 
    every upper boundary component/.style={draw, dashed},
    cobordism edge/.style={draw}];
    \pic[tqft cylinder, anchor=outgoing boundary, at=(a-incoming boundary), 
    ...
    \pic[tqft cylinder, anchor=incoming boundary, at=(a-outgoing boundary 1), 
    every outgoing lower boundary component/.style={draw},
    ...
    \pic[tqft cylinder, anchor=incoming boundary, at=(a-outgoing boundary 2), 
    every outgoing lower boundary component/.style={draw},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate}, 
    every outgoing boundary component/.style={fill=ibmBlue, fill opacity=.6},
    every outgoing boundary component/.style={fill=ibmBlue, fill opacity=.4},
    cobordism edge/.style={draw},
    cobordism height=0.2cm, fill=ibmBlue, fill opacity=.6];
    % \pic[tqft cylinder, rotate=90, at={(-5,-1)}, cobordism height=0.2cm,
    % every incoming upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    % fill=ibmYellow, fill opacity=.4,
    % every outgoing boundary component/.style={dashed, draw, fill=ibmYellow, fill opacity=.8},
    % cobordism edge/.style={draw},
    % ];
    \draw [thick, right hook->, ibmYellow] (-3.5,-1) -- node [above=.5em,align=center, color=ibmYellow] {$i_0$} (-2,-1);
    \pic[tqft cylinder, rotate=-90, at={(-4,-1)}, cobordism height=0.2cm,
    every outgoing lower boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    ...
    cobordism edge/.style={draw}];
    \node at (-4, -2) {$\mathbb{S}^1 \times [0, \varepsilon)$};
    \draw [thick, left hook->, ibmBlue] (3.5,-1) -- node [above=.5em,align=center, color=ibmBlue] {$i_1$} (2,-1);
    \pic[tqft cylinder, rotate=90, at={(4,-1.5)}, cobordism height=0.2cm, fill=ibmBlue, fill opacity=.6,
    every incoming lower boundary component/.style={dashed, draw},
    every incoming upper boundary component/.style={dashed, draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    every outgoing boundary component/.style={draw, fill=ibmBlue, fill opacity=.4},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    cobordism edge/.style={draw}];
    \pic[tqft cylinder, rotate=90, at={(4,-.5)}, cobordism height=0.2cm, fill=ibmBlue, fill opacity=.6,
    every incoming lower boundary component/.style={dashed, draw},
    every incoming upper boundary component/.style={dashed, draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    every outgoing boundary component/.style={draw, fill=ibmBlue, fill opacity=.4},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    cobordism edge/.style={draw}];
    \node at (4, -2.5) {$(\mathbb{S}^1 \sqcup \mathbb{S}^1) \times (1- \varepsilon, 1]$};
    \draw [line width=1mm, -{Stealth[length=4mm]}](0.3,-.4) -- (.3,-1.3);
  \end{tikzpicture}
\end{figure}

The figure shows how this choice of intervals we made in the definition assures that the inclusion maps are both orientation preserving.

We can also see that in-boundaries and out-boundaries do not depend on the orientation of the manifold $M$, but only on the two inclusion maps. For example, we could consider the same manifold $M$ and equip it with two in-boundaries and one out-boundary, as depicted below. From now on we will draw in-boundaries on the left and out-boudaries on the right, to avoid further confusion.
...
\begin{figure}[H]
  \centering
   \begin{tikzpicture}[every tqft/.style={transform shape}]
    ...
    cobordism height=0.2cm, fill=ibmBlue, fill opacity=.6];
    \draw [thick, right hook->, ibmYellow] (-3.5,-1) -- node [above=.5em,align=center, color=ibmYellow] {$i_0$} (-2,-1);
    \pic[tqft cylinder, rotate=-90, at={(-4,-.5)}, cobordism height=0.2cm,
    every outgoing lower boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    fill=ibmYellow, fill opacity=.6,
    every outgoing boundary component/.style={fill=ibmYellow, fill opacity=.4},
    every incoming boundary component/.style={dashed, draw},
    cobordism edge/.style={draw}];
    \pic[tqft cylinder, rotate=-90, at={(-4,-1.5)}, cobordism height=0.2cm,
    every outgoing lower boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    fill=ibmYellow, fill opacity=.6,
    every outgoing boundary component/.style={fill=ibmYellow, fill opacity=.4},
    every incoming boundary component/.style={dashed, draw},
    cobordism edge/.style={draw}];
    \node at (-4, -2.5) {$(\mathbb{S}^1 \sqcup \mathbb{S}^1) \times [0, \varepsilon)$};
    \draw [thick, left hook->, ibmBlue] (3.5,-1) -- node [above=.5em,align=center, color=ibmBlue] {$i_1$} (2,-1);
    \pic[tqft cylinder, rotate=90, at={(4,-1)}, cobordism height=0.2cm, fill=ibmBlue, fill opacity=.6,
    every incoming lower boundary component/.style={dashed, draw},
    every incoming upper boundary component/.style={dashed, draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    every outgoing boundary component/.style={draw, fill=ibmBlue, fill opacity=.4},
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    cobordism edge/.style={draw}];
    \node at (4, -2) {$\mathbb{S}^1 \times (1- \varepsilon, 1]$};
  \end{tikzpicture}
\end{figure}
\end{example}
...
\todo[inline, color=yellow]{{\bfseries Marco cattivo}: Nn hai citato TASI lectures.}
%\todo[inline, color=yellow]{{\bfseries Marco cattivo}: Potresti citare la tesi di phd ti Tizio.}

\begin{example}
  We illustrate such procedure with the following example, considering bordisms of dimension 2. We take a bordism $(X, i_0, i_1)$ from $\Sigma_0 = \mathbb{S}^1 \sqcup \mathbb{S}^1$ to $\Sigma_1 = \mathbb{S}^1$ and a bordism $(X', i'_0, i'_1)$ from $\Sigma_1 = \mathbb{S}^1$ to $\Sigma_2 = \mathbb{S}^1$ and glue them along the common boundary $\Sigma_1$. Since our goal is be to define a sort of ``composition'' we will always glue an out-boundary to an in-boundary.
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[every tqft/.style={transform shape}]
      \node at (-2.2, -.35) {$X$};
...
More is actually true when considering the gluing of two oriented bordisms.
...
Our goal is now to construct a category of oriented $ n $-dimensional bordisms. The intuitive idea behind it is to take closed oriented $(n-1)$-dimensional manifolds as objects and oriented bordisms between them as morphisms. The proper definition, however, requires some more refinement.  We begin by adressing some techinical issues. While equipping each bordism with an explicit choice of a collar allows us to properly define a gluing, this approach fails when trying to define a strict identity morphism. Given a closed oriented $(n-1)$-dimensional manifold $\Sigma$, a candidate for $\id_\Sigma$ is given by the cylinder $\Sigma \times [0,1]$ with some choice of collars. However, gluing it to a bordism $M$ with one of the boundaries equal to $\Sigma$, only gives us a manifold \emph{diffeomorphic} to $M$. By considering the underlying topological spaces we easily understand that the only way to define a strict identity in this setting is to consider $\Sigma$ itself as a bordism, but this would not satisfy our definition which requires it to be an $n$-dimensional manifold. Finally we recall how, when gluing manifolds, different choices of collars give rise to different but diffeomorphic smooth structures. This motivates the following framework: we define the bordism category by taking as morphisms bordisms ``up to diffeomorphism''.
...
\end{tcbprp}
\begin{proof}
  Recall that $f \colon \Sigma_0 \to \Sigma_1$, $g \colon \Sigma_0 \to \Sigma_1$ are (smoothly) homotopic if there exists a smooth map $\phi \colon \Sigma_0 \times [0,1] \to \Sigma_1$ such that $\phi(x, 0) = f(x)$ and $\phi(x, 1) = g(x)$. Equivalentely we are asking for the following diagram to be commutative. 
  \[
  \begin{tikzcd}
  ...
  \end{tikzcd}
  \]
  Even if the naming of the arrows is a bit sloppy, this define the equivalence between the two bordisms. 
...
Note that in the article we do not encounter the explicit notion of bordisms as we defined them. 

Although the interest in the topic mainly comes from physics we will, as mentioned in the introduction, ignore that perspective eintirely.

We here give a slightly refined version (which can be found in \todo{cite:cocco}) of the original axioms, stated in terms of bordisms.
...
Indeed, axiom \ref{tqft:a1} well defines a map form $\bord{n}$ to $\mathbf{Vect}_\Bbbk$ and axioms \ref{tqft:a2} and \ref{tqft:a3} guarantee such map is really a functor.
...
\begin{dfnx*}[A brief digression on dualizability]
  We now leave some space to discuss an important property regarding TQFTs, starting with what is known as the \emph{snake decomposition of a cylinder}.
  ...
  \end{tikzpicture}
  \]
  Notice how, after the cutting, the circle in the middle won't be equipped with the usual orientation induced by the product, but the opposite one \footnote{More precisely, we should say that, considering the bordism $\beta$ we have }.
  \todo[inline]{Sistemare orientazione} 
  We will denote it as $\overline{\Sigma}$.
  This type of decomposition is called the \emph{snake decomposition} and can be written as compositions of arrows as
  \[
  \Sigma \iso \Sigma \sqcup \varnothing \xrightarrow{\id_\Sigma \sqcup \gamma} \Sigma \sqcup \overline{\Sigma} \sqcup \Sigma \xrightarrow{\beta \sqcup \id_\Sigma} \varnothing \sqcup \Sigma \iso \Sigma 
  \]
  \[
  \Sigma \iso \varnothing \sqcup \Sigma \xrightarrow{\gamma \sqcup \id_\Sigma} \Sigma \sqcup \overline{\Sigma} \sqcup \Sigma \xrightarrow{\id_\Sigma \sqcup \beta} \Sigma \sqcup \varnothing \iso \Sigma 
  \]
  where $\beta \colon \Sigma \sqcup \overline{\Sigma} \to \varnothing$ and $\gamma \colon \varnothing \to \overline{\Sigma} \sqcup \Sigma$ are the ``bent cylinders''. We can then state that:
  \[
  \id_\Sigma \iso (\beta \sqcup \id_\Sigma) \circ (id_\Sigma \sqcup \gamma) 
  \hspace{3em} \text{and} \hspace{3em}
  \id_\Sigma \iso (\id_\Sigma \sqcup \beta) \circ (\gamma \sqcup \id_\Sigma)
  \]
  ...
  \hspace{3em}   
  Z(\beta) \colon V \tensor \overline{V} \to \Bbbk
  \hspace{3em}
  Z(\gamma) \colon \Bbbk \to \overline{V} \tensor V
  \]
  \begin{tcblemma}
    The image vector space of a TQFT is of finite dimension.
  \end{tcblemma}
  \begin{tcblemma}
...
We now give a couple example of some simple TQFTs.
...
\begin{tcbex}[A 2-dimensional TQFT computing the genus]
  ...
  \]
  Consider now a surface with two holes $2\mathbb{T}^2$ which we can draw by gluing the above bordisms as $\begin{tikzpicture}[every tqft/.style={transform shape}, scale=0.35, baseline=-.25ex, tqft/boundary separation=1.5cm]
        \pic[tqft cap, name=a, rotate=90, draw, every outgoing lower boundary component/.style={dashed, draw}];
        \pic[tqft pair of pants, name=b, rotate=90, draw, anchor= incoming boundary, at=(a-outgoing boundary), every outgoing lower boundary component/.style={dashed, draw}];
        \pic[tqft reverse pair of pants, name=c, rotate=90, draw, anchor= incoming boundary 1, at=(b-outgoing boundary 1), every outgoing lower boundary component/.style={dashed, draw}];
    ...
    \[
    Z(2\mathbb{T}^2) \colon 1_\Bbbk \mapsto v_1 \mapsto v_0 \tensor v_0 + v_1 \tensor v_1 \mapsto 2v_1 \mapsto 2(v_0 \tensor v_0 + v_1 \tensor v_1) \mapsto 2(2v_1) \mapsto 4 \, (= 2^2)
    \]
