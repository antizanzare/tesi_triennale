...
\begin{example}
Let us draw an example of a 2-dimensional bordism. We hence consider a 2-dimensional oriented manifold $M$ homeomorphic to a spere with three holes (also called a \emph{pair of pants}) and the corresponding maps $i_0$, $i_1$, defining its orientation as a bordism.
\begin{figure}[h]
  \centering
...
By looking at the drawing we can see the reasoning behind our definition of inclusion maps. The way we chose intervals lets us say that the inclusions are both orientation preserving.
  ...
  %   \end{tikzpicture}
  % \end{figure}
  % \todo[inline]{sistema e/o cancella questo orrore in futuro}
\end{example}

The role of the embeddings $i_0$, $i_1$ in the above definition is to identify a collar of the in-boundary and the out-boundary. To make this precise, we again go back to differential geometry \cite{smooth}.

\todo[inline, color=yellow]{{\bfseries Marco cattivo}: ``We again go back to differential geometry'' è un po' clumsy sec me.}
