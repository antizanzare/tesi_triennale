...
\begin{tcbdfn}[Oriented bordisms]
  ...
  \item two orientation preserving embeddings
    ...
    \] 
    definining an \emph{in-boundary} $(\partial M)_0 \defeq i_0(\Sigma_0, 0)$ and an \emph{out-boundary} $(\partial M)_01\defeq i_1(\Sigma_1, 1)$, such that $\partial M = (\partial M)_0 \sqcup (\partial M)_1$.
  \end{itemize}
\end{tcbdfn}
...
\begin{figure}[ht]
  \centering
   \begin{tikzpicture}[every tqft/.style={transform shape}]
    ...
    cobordism edge/.style={draw},
    cobordism height=0.2cm, fill=ibmMagenta, fill opacity=.6];
    ...
    % every incoming upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{<}},}, postaction=decorate},
    % fill=ibmMagenta, fill opacity=.4,
    % every outgoing boundary component/.style={dashed, draw, fill=ibmMagenta, fill opacity=.8},
    % cobordism edge/.style={draw},
    % ];
    \draw [thick, right hook->, ibmMagenta] (-3.5,-1) -- node [above=.5em,align=center, color=ibmMagenta] {$i_0$} (-2,-1);
    ...
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    fill=ibmMagenta, fill opacity=.6,
    every outgoing boundary component/.style={fill=ibmMagenta, fill opacity=.4},
    ...
    every outgoing upper boundary component/.style={draw, decoration={markings, mark=at position .5 with {\arrow{>}},}, postaction=decorate},
    cobordism edge/.style={draw}];
    \node at (4, -2.5) {$(\mathbb{S}^1 \disj \mathbb{S}^1) \times (1- \varepsilon, 1]$};
  \end{tikzpicture}
\end{figure}
...
We can also see that in-boundaries and out-boundaries do not depend on the orientation of the manifold $M$, but only on the two inclusion maps. For example, we could consider the same manifold $M$ and equip it with two in-boundaries and one out-boundary, as depicted below.
\begin{figure}[H]
  \centering
   \begin{tikzpicture}[every tqft/.style={transform shape}]
    \pic[name=a, tqft, offset=-0.7, incoming boundary components=1,
    ...
    every incoming boundary component/.style={dashed, draw},
    cobordism edge/.style={draw}];
    \node at (-4, -2.5) {$(\mathbb{S}^1 \disj \mathbb{S}^1) \times [0, \varepsilon)$};
    \draw [thick, left hook->, ibmBlue] (3.5,-1) -- node [above=.5em,align=center, color=ibmBlue] {$i_1$} (2,-1);
...
The role of the embeddings $i_0$, $i_1$ in the above definition is to put some sort of ``collar'' around the in-boundary and the out-boundary.
...
\begin{example}
  We illustrate such procedure with the following example in dimension 2. Take a bordism $(X, i_0, i_1)$ from $Y_0 = \mathbb{S}^1 \disj \mathbb{S}^1$ to $Y_1 = \mathbb{S}^1$ and a bordism $(X', i'_0, i'_1)$ from $Y_1 = \mathbb{S}^1$ to $Y_2 = \mathbb{S}^1$. 
  \begin{figure}[H]
    \centering
    \begin{tikzpicture}[every tqft/.style={transform shape}]
      \node at (-2.2, -.35) {$X$};
...
\todo[inline]{see lemma ch. 1 (prod, coprod and monoidal cat)}
% \begin{tcblemma} 
...
Axioms A\ref{tqft:a4} and A\ref{tqft:a5} preserve the monoidal structure. To get a \emph{symmetric} monoidal functor, as defined in \ref{dfn:tqft}, we would have to add a sixth axiom:
\begin{enumerate}
