...
\begin{tcbdfn}
The category of oriented n-dimensional bordisms $\bord$ is defined as follows.
\begin{itemize}
  \item The objects are closed oriented $(n-1)$-dimensional manifolds
  \item For any $\Sigma_0, \Sigma_1 \in \obj{\bord}$, morphisms are the equivalence classes of bordisms $M \colon \Sigma_0 \to \Sigma_1$
...
\begin{dfnx}[Disjoint union of cobordisms]
Given two bordisms $M \colon \Sigma_0 \to \Sigma_1$ and $N \colon \Sigma'_0 \to \Sigma'_1$, their disjoint union $M \amalg N$ naturally defines a bordism from $\Sigma_0 \amalg \Sigma'_0$ to $\Sigma_1 \amalg \Sigma'_1$. 
...
Then $(\cat{C}, \amalg, 0)$, where $\amalg$ denotes the coproduct and $0$ the initial object, is a monoidal category.
\end{tcblemma}

%\begin{proof} \todo[inline]{is this needed?}
%Firstly, let us prove the associativity axiom. Consider any objects $A$, $B$, $C$ in $\cat{C}$ and show that $(A \amalg B) \amalg C \iso A \amalg (B \amalg C)$.
%We hence take the objects $(A \amalg B) \amalg C$, $A \amalg (B \amalg C)$ and the corresponding inclusion maps. By the universal property of the coproduct $A \amalg B$ we have a unique map $A \amalg B \to A \amalg (B \amalg C)$ and
%\[
%by the universal property of the coproduct $(A \amalg B) \amalg C$, there exists a unique morphism $(A \amalg B) \amalg C \to A \amalg (B \amalg C)$.
%Similarly, by the universal property of $B \amalg C$ and $A \amalg (B \amalg C)$, we have a unique morphism $A \amalg (B \amalg C) \to (A \amalg B) \amalg C$. By uniqueness of such maps we have the isomorphism we wished for.
%\[
%\begin{tikzcd}[column sep=huge, row sep=small]
%& A \amalg (B \amalg C) \arrow[dddd, bend left= 30, dashed] &\\
%A \arrow[ur, tail] \arrow[dd, tail] & & B \amalg C \arrow[ul, tail] \arrow[dddl, dashed, "\exists !"] \\
%& B \arrow[ur, tail] \arrow[dl, tail] & \\
%A \amalg B \arrow[dashed, uuur, "\exists !"] \arrow[dr, tail] & & C \arrow[dl, tail] \arrow[uu, tail] \\
%& (A \amalg B) \amalg C \arrow[uuuu, bend left= 30, dashed] & 
%\end{tikzcd}
%\]
%We now verify the unit axiom. %What we'll prove is that $A$ satisfies the universal property of the coproducts $A \amalg 0$ and $0 \amalg A$.
%Again in the diagram below we have that by the univ property of $0 \amalg A$ there exists a unique arrow making the diagram commute. We can show such arrow is the inverse of the inclusion (again by univ property of $0 \amalg A$)
%\[
%\begin{tikzcd}
%& 0 \amalg A  \arrow[dd]& \\
...
%\end{tikzcd}
%\]
%Similarly, this holds when considering $A \amalg 0$.
%Having defined these isomorphisms through the universal property we can be sure the coherence axioms are satisfied too.
%\end{proof}

\begin{tcbprp}
The category $\bord$ has a monoidal structure given by the disjoint union of manifolds (and the initial object, being the empty manifold $\varnothing$).
%$(\bord, \amalg, \emptyset)$ is a monoidal category. (or better, is a monoidal structure on $\bord$)
\end{tcbprp}

\begin{tcbprp}
Any diffeomorphism of $(n-1)$-dimensional manifolds $\Sigma_0$, $\Sigma_1$ define an equivalence class of bordisms $M \colon \Sigma_0 \to \Sigma_1$.
\end{tcbprp}

\begin{dfnx}[The twist bordism]
The twist diffeomorphism of manifolds $\sigma \colon \Sigma \amalg \Sigma' \to \Sigma' \amalg \Sigma$ defines a cobordism in $\bord$ which we'll denote as
$T_{\Sigma,\Sigma'} \colon \Sigma \amalg \Sigma' \to \Sigma' \amalg \Sigma$. 
\end{dfnx}

\begin{tcbprp}
The category $\bord$ has a symmetric monoidal structure $(\bord, \amalg, \varnothing, T)$
\end{tcbprp}

\section{Topological Quantum Field Theories}

\begin{tcbdfn}
An \emph{$n$-dimensional topological quantum field theory} is a symmetric monoidal functor from $(\bord, \amalg, \varnothing, T)$ to $(\mathbf{Vect}, \otimes, \Bbbk, \sigma)$
\end{tcbdfn}

Historically, the first axiomatization of TQFTs appeared in the late 1980s. The first explicit set of axioms was given by Atiyah in 1988.
Let us see how they compare with the functorial definition we just stated.

\begin{tcbdfn}[Atiyah's axioms of a TQFT]
An n-dimensional topological quantm field theory consists of the following data:
\begin{itemize}
